...
| M54 — Intel & balance (stale structure snapshots + garrison beliefs for player and AI alike; fog-symmetric AI assault counsel; belief-error battle reports; personality-tag castle templates; `npm run bench:assault` matrix — drove the fractional-casualty accumulator and the keep's last-stand rally) | ✅ this commit — **Phase 8 complete: The Castle shipped end to end** |
| M55 — Single siege path (`siege.begin` gated on a standing defence layer; legacy breach-gated assault branch, `siege.setTarget`, `breaches`/`targetBuilding` deleted; siege save v2→v3) | ✅ — **Phase 8.1 begins: One Castle** (ADR-4 Amendment A1) |
| M56 — Retire village-side fortification (`category: 'castle'` rejected in the one placement rulebook; `planCastleRing`, castles.ts enclosure/`isCastle` derivation deleted) | ✅ — one castle system, structurally: `game/castles.ts` is gone, not merely unreachable |
| M57 — Keep-gated derived layers (layer re-keyed kingdom→VILLAGE; keep recategorised `castle`→`military`; templates additive by tier; defence `SettlementNeed` replaces `planCastleRing`) | ✅ — any keep-bearing village is assault-resolved; capital status stays POLITICAL |
| M58 — Keep-panel repair (`def.cost × (1 − hp/maxHp)`, blocked during a siege, paid from village stores, `repairingUntil` window; mirrored in the AI daily manager) | ✅ — closes the pre-existing M51 gap where damage never healed |
| M59 — Footprints, readability & zoom (Keep 7×7, Tower 3×3, Gatehouse 3×2 + orientation twin; hp re-scaled per frontage; toughness-aware `pickWallTarget` gives the gatehouse a real role; ~3× tile scale with pan) | ✅ — a balance change in a rendering change's clothes; `bench:assault` re-run with it |
| M60 — Legacy saves & corpus (`RETIRED_SYSTEM_NAMES` tolerance so M28-era saves load at all; grandfathered walls load inert; standing structures occupy the footprint they were PLACED with) | ✅ — new `legacy-castle-m28-v1` corpus entry; two A1 predictions corrected by execution |
| M61 — Docs & balance recert (GDD §7 rewritten as one system; doc 06 §4 `isCastle`/`DefenseGraph` retirement; doc 07 §5 delta) | ✅ — **Gate P8.1 verified 2026-07-23: Phase 8.1 closed** |
| M61.5 — Release review (M1–M61) | ✅ — verdict **Not Ready**: four simulation blockers named (no player expansion verb · inverted demographic pyramid · war that never concludes · victory monoculture); art gap closed by repositioning (ADR-6). Charters **Phase 9** (doc 12 revision R5) |
| M62 — Balance instrumentation as a gate (`bench:balance --real` asserts four outcome BANDS; `balance-matrix` CI job) | ✅ — **Phase 9 begins: The Game**. Bands ratified before the fixes they gate; confirmed failing on all four against HEAD (expected red until M65/M66) |
| M63 — The player's missing verbs (settler dispatch from the Village panel; the missing ownership guard on `village.sendSettlers`; named save slots + a plain slot list) | ✅ — the player is no longer a permanent one-village kingdom; hash-inert |
| M64a — Demographic diagnosis | ✅ — **finding, not a fix**: `population.ts` is a net CONTRIBUTOR of +309..+373 adults/village over 20y. The ~20:1 child-to-adult inversion is caused by AI recruiting at 10 adults/unit against floor-only gates. Per-village term accounting reconciles to the decimal |
| ~~M64b~~ | merged into M65 by roadmap revision R6 — the demographic inversion and the never-massing army are ONE defect |
| M65 — The military economy (workforce ceiling + affordability ceiling on recruiting; peacetime tax baseline NORMAL, reversing the prior no-baseline policy) | ✅ — adult-cohort band 6.8%→45.1% and oldest-village floor 5.1%→27.9%, both GREEN. First territorial victory the real composition has ever produced. Goldens/corpus re-recorded intentionally (2 fixtures) |
| M66 — Victory semantics & pacing (Conquest → **elimination-only**; Prosperity re-paced 80→75 joy, 60→90 heads) | ✅ — monoculture band chronicle 94%→prosperity 56%, GREEN. ADR-5's taken-village share was built, measured at 0 wins in 16 campaigns, and deleted. No fixture movement (victory state is not hashed) |
| M67 — Onboarding, docs & Gate P9 | 🔄 in progress — ADR sweep for the five undocumented cuts (ADR-7…ADR-11), doc 07 §3 stale-note correction, this table, reachability tutorial, Gate P9 |
