...
export function composeTerra(
  ...
  kernel.registerCommand<{ x: number; y: number; biomeCode: number }>('sandbox.editTerrain', (ctx, payload, command) => {
    ...
    if (code === Biome.Ocean || code === Biome.Coast) {
      return rejectSandbox(ctx, 'sandbox.editTerrain', 'cannot paint water (land-only edits)', command.issuer);
    }
    const i = y * width + x;
    const oldCode = biome[i] as number;
    if (oldCode === Biome.Ocean || oldCode === Biome.Coast) {
      return rejectSandbox(ctx, 'sandbox.editTerrain', 'cannot repaint a water tile', command.issuer);
    }
